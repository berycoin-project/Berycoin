...
void SendTokenPage::on_numBlocksChanged()
{
    if(m_clientModel)
    {
        ...
        ui->labelGasPrice->setToolTip(tr("Gas price: BERY price per gas unit. Default = %1, Min = %2.").arg(QString::fromStdString(FormatMoney(DEFAULT_GAS_PRICE))).arg(QString::fromStdString(FormatMoney(minGasPrice))));
...
void SendTokenPage::on_confirmClicked()
{
    ...
    {
        QString message = tr("To send %1 you need BERY on address <br /> %2.")
